...
/////////////////////////////////////////////////////////////////////////////
// QR版本信息
static QR_VERSIONINFO QR_VersonInfo[] = {{0}, // (版本:Ver.0)
										 { 1, // Ver.1
										    26,   19,   16,   13,    9,
										   0,   0,   0,   0,   0,   0,   0,
										   1,  26,  19,
...
int CQR_Encode::EncodeData(int nLevel, int nVersion, int bAutoExtent, int nMaskingNo, const char * lpsSource, int ncSource)
{
	int i, j;
...
memset(m_nBlockLength,0,sizeof(m_nBlockLength));
	...
	for (m_ncDataBlock = i = 0; i < ncLength; ++i)
	{
		...
		if (i < ncLength - 1 && IsHanjiData(lpsSource[i], lpsSource[i + 1]))
...
memset(m_byDataCodeWord,0,MAX_DATACODEWORD);
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		...
		else if (m_byBlockMode[i] == QR_MODE_ALPHABET)//字母模式
		{
		...
		else if (m_byBlockMode[i] == QR_MODE_8BIT)//字节流模式
		{
		...
		else // 汉字模式
		{
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned short)(m_nBlockLength[i] / 2), nIndicatorLenHanzi[nVerGroup]);
...
int CQR_Encode::IsNumeralData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return 1;
...
int CQR_Encode::IsAlphabetData(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return 1;
...
/////////////////////////////////////////////////////////////////////////////
// CQR_Encode::IsHanjiData


int CQR_Encode::IsHanjiData(unsigned char c1, unsigned char c2)
{
