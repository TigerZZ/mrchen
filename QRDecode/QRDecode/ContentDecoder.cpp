...
BOOL ContentDecoder::DecodeData(int nCodeSize,int nVersion,BYTE CodeData[MAX_MODULESIZE][MAX_MODULESIZE])
{
	...
	/*
		标记非数据位置
	*/
	SetFinderPattern();//标记位置探测图形
	if(m_nVersion>1)
		SetAlignmentPattern();//版本大于1的话会有校正图形,也标记
	SetFormatPattern();
	SetVersionPattern();
...
void ContentDecoder::GetCodeWord()
{
	...
	for(i=0;i<m_ncAllCodeWord;i++)//遍历码字
	{
		for(j=0;j<8;j++)//每个码字长度为8
		{
			do
			{
				x+=nCoef_x;
...
void ContentDecoder::ParseDataCodeWord()
{
	...
	for(i=0;i<m_ncDataCodeWord && m_nIndex != -1; i++)
	{
		...
		{
			wIndicator=GetBitStream(nIndicatorLenNumeral[m_nVersionGroup]);//根据版本与模式返回的字节流,wIndicator的值表示后面跟的数字字符的数目
			for(j=0;j<wIndicator;j+=3)//每隔3位一组，
			{
				if(j<wIndicator-2)
				{
...
//读取格式信息，返回纠错级别、掩膜号的联合信息,共5位，前两位为纠错级别，后////三位为掩膜号
